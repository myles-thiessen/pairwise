...
public class ExperimentTraceExtractor {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(ExperimentTraceExtractor.class);
  ...
  private static void getTraces(final String dir) throws IOException {
    List<Map<Pair<Object, Integer>, IOperationMeasurement>> traces =
        new LinkedList<>();
    for (int i = 0; i < N; i++) {
  ...
  private static void printTrace(
      final String filename, final Map<Pair<Object, Integer>,
      IOperationMeasurement> traceMap)
      throws IOException {
    System.out.println(filename);
    File file = new File(filename);
    file.createNewFile();
    try (FileWriter writer = new FileWriter(file)) {
      for (Map.Entry<Pair<Object, Integer>, IOperationMeasurement> entry :
          traceMap.entrySet()) {
        ...
        for (Map.Entry<String, Long> stageEntry : stages.entrySet()) {
          ...
          String result =
              String.format(
                  "%s %s %d %s %d",
                  opType, identifier, opNumber, stageEntry.getKey(),
                  stageEntry.getValue());
