...
public class ThroughputReadClient {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(ThroughputReadClient.class);
  ...
  public void start() {
    ...
    ScheduledFuture<?> scheduledFuture =
        Util.SCHEDULED_EXECUTOR_SERVICE.scheduleAtFixedRate(this::status, 5,
            5, TimeUnit.SECONDS);
  ...
  private void clientWork() {
    ...
    if (this.measure)
      METRICS_REPORTER.report(
          METRICS_REPORTER.getUniqueNumberGenerator().getUniqueNumber(), "OPS"
          , this.opsCompleted);
