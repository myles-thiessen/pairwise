...
public class ExperimentYCSBRealWorldMaster {

  private static final Logger LOGGER = LoggerFactory.getLogger(ExperimentYCSBRealWorldMaster.class);
  ...
  private static void startYCSB() {
    ...
    for (int i = 0; i < N; i++) {
      Util.Node node = NODES.get(i);
      Session session = SSH.get(i);
      String readWorkload = "workload-read";
      Util.CACHED_THREAD_POOL.submit(
          () -> {
            try {
              ChannelExec channel = (ChannelExec) session.openChannel("exec");
