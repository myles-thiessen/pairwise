...
public class ExperimentNetworkVariabilityMaster {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(ExperimentNetworkVariabilityMaster.class);
  ...
  private static void increaseLatency() {
    String leaderIp = NODES.get(0).ip();
    String follower1Ip = NODES.get(1).ip();
    String follower2Ip = NODES.get(2).ip();
    ...
    NetworkOuterClass.SetLatencyRequest leaderRequest =
        NetworkOuterClass.SetLatencyRequest.newBuilder()
            .putLatency(follower2Ip, 80)
            .putLatency(follower1Ip, 16)
            .build();
    leaderStub.updateLatency(leaderRequest);

    ExperimentGrpc.ExperimentBlockingStub follower1Stub = STUBS.get(1);
    NetworkOuterClass.SetLatencyRequest follower1Request =
        NetworkOuterClass.SetLatencyRequest.newBuilder()
            .putLatency(leaderIp, 16)
            .putLatency(follower2Ip, 64)
            .build();
    follower1Stub.updateLatency(follower1Request);

    ExperimentGrpc.ExperimentBlockingStub follower2Stub = STUBS.get(2);
    NetworkOuterClass.SetLatencyRequest follower2Request =
        NetworkOuterClass.SetLatencyRequest.newBuilder()
            .putLatency(leaderIp, 80)
            .putLatency(follower1Ip, 64)
            .build();
    follower2Stub.updateLatency(follower2Request);

    if (PROTOCOL.startsWith("DEL")) {
      for (int i = 0; i < N; i++) {
        ExperimentGrpc.ExperimentBlockingStub stub = STUBS.get(i);
        NetworkOuterClass.UpdateDeltaRequest updateDeltaRequest =
            NetworkOuterClass.UpdateDeltaRequest.newBuilder().setDelta(67).build();
        stub.updateDelta(updateDeltaRequest);
      }
    }
  }

  private static void revertLatency() {
    String leaderIp = NODES.get(0).ip();
    String follower1Ip = NODES.get(1).ip();
    String follower2Ip = NODES.get(2).ip();
    ...
    if (PROTOCOL.startsWith("DEL")) {
      for (int i = 0; i < N; i++) {
        ExperimentGrpc.ExperimentBlockingStub stub = STUBS.get(i);
        NetworkOuterClass.UpdateDeltaRequest updateDeltaRequest =
            NetworkOuterClass.UpdateDeltaRequest.newBuilder().setDelta(27).build();
        stub.updateDelta(updateDeltaRequest);
      }
    }
  }
  ...
  private static void startUniformWorkload() {
    ...
    long update = start + 5000 + 500 + 250;
    long revert = update + 5000;
    ...
    long wait = update - System.currentTimeMillis();
    if (wait > 0) {
      try {
        Thread.sleep(wait);
      } catch (InterruptedException e) {
    ...
    System.out.println("Increased latency");

    wait = revert - System.currentTimeMillis();
    if (wait > 0) {
      try {
        Thread.sleep(wait);
      } catch (InterruptedException e) {
    ...
    System.out.println("Reverted latency");
