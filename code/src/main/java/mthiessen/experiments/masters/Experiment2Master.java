...
public class Experiment2Master {

  private static final Logger LOGGER = LoggerFactory.getLogger(Experiment2Master.class);
  ...
  private static void startUniformWorkload() {
    ...
    // leader request
    NetworkOuterClass.StartWorkloadRequest leaderRequest =
    ...
    for (int i = 0; i < N; i++) {
      ExperimentGrpc.ExperimentBlockingStub stub = STUBS.get(i);
      NetworkOuterClass.StartWorkloadRequest request = i == 0 ? leaderRequest : readerRequest;
      Util.CACHED_THREAD_POOL.submit(
          () -> {
