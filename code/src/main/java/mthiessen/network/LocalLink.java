...
public class LocalLink implements ILink {
  ...
  private final ExecutorService forwardService =
      Executors.newSingleThreadExecutor();
  private final ExecutorService commitService =
      Executors.newSingleThreadExecutor();
  private final ExecutorService regularService =
      Executors.newSingleThreadExecutor();

  private ExecutorService getService(final Object operations) {
    ExecutorService service;
    switch ((String) operations) {
      case OPS.FORWARD_WRITE, OPS.FORWARD_READ -> service = this.forwardService;
