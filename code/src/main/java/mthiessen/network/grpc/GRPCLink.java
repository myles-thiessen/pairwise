...
public class GRPCLink implements ILink {
  ...
  private void run(final Object operation,
                   final NetworkOuterClass.Message message) {
    this.stub.handleRequest(
        ...
        new StreamObserver<>() {
          ...
          public void onNext(final NetworkOuterClass.Message message) {
            Object sender =
                SerializationUtils.deserialize(message.getSender().toByteArray());
            Object operation =
                SerializationUtils.deserialize(message.getOperation().toByteArray());
            int broadcastRequestNumber = message.getBroadcastRequestNumber();
            long time = message.getTime();
            Object response =
                SerializationUtils.deserialize(message.getPayload().toByteArray());
            handleResponse(sender, operation, broadcastRequestNumber, time,
                response);
          }
  ...
  public void handleResponse(
    ...
    this.router.handleResponse(sender, operation, broadcastRequestNumber,
        time, response);
  }
