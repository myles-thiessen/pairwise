...
public class NetworkService extends NetworkGrpc.NetworkImplBase {
  ...
  public void handleRequest(
    ...
    Object sender =
        SerializationUtils.deserialize(request.getSender().toByteArray());
    Object operation =
        SerializationUtils.deserialize(request.getOperation().toByteArray());
    int broadcastRequestNumber = request.getBroadcastRequestNumber();
    long time = request.getTime();
    Object requestPayload =
        SerializationUtils.deserialize(request.getPayload().toByteArray());
