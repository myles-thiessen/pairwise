...
public class StoppedRequestHandler implements IRequestHandler {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(StoppedRequestHandler.class);
  ...
  @NonNull
  private final StartTimeManager stoptimeManager;

  @NonNull
  private final IRequestHandler commitRequestHandler;
  ...
  public Object execute(final Object sender, final Object request) {
    ...
    long stopped = this.eventSchedulingPrimitive.event(sender,
        stopTimeRequest.stopped());

    boolean done = this.stoptimeManager.record(sender,
        stopTimeRequest.index(), stopped);

    if (done) {
      int index = stopTimeRequest.index();

      Payloads.WriteRequest writeRequest =
          this.writeRecorder.waitAndGetRequest(index);

      MetricsAttachedRequest<Payloads.WriteRequest> metricsAttachedRequest =
          new MetricsAttachedRequest<>(writeRequest, new Pair<>(0, 0));
