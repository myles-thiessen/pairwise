...
import mthiessen.protocol.reads.LeaderReads;
...
public class LeaderReadsAlgorithm extends BaseProcess {
  ...
  public static IProcess generate(
    ...
    return new LeaderReadsAlgorithm(
        router,
        measurementCollector,
        leaderElectionOp,
        leaderElectionService,
  ...
  public void initialize() {
    ...
    routerState.registerResponseHandleForOp(
        OPS.WRITE,
        () -> new WriteResponseHandler(numProcesses, majority, 0,
    ...
    routerState.registerRequestHandlerForOp(OPS.FORWARD_READ,
        () -> forwardReadRequestHandler);

    // Needs to be unique per broadcast id.
    routerState.registerResponseHandleForOp(OPS.FORWARD_READ,
        ForwardReadResponseHandler::new);
  }
}
